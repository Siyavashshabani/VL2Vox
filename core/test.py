...
def test_net_flava(cfg,
    ...
    for sample_idx, (taxomony_class, taxonomy_id, sample_name, rendering_images, ground_truth_volume) in enumerate(test_data_loader):
        ...
        with torch.no_grad():
            ...
            # print("FLAVA_outputs---------------------------------test:", FLAVA_outputs.multimodal_embeddings.shape)

            FLAVA_outputs = ensure_fixed_shape(FLAVA_outputs.multimodal_embeddings, target_length=202)

            FLAVA_outputs_reshaped = convert_layer(FLAVA_outputs )

            # Test the encoder, decoder, refiner and merger
            image_features = encoder(rendering_images)
            raw_features, generated_volume = decoder(image_features+ FLAVA_outputs_reshaped )
